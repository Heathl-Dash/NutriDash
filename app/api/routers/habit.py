...
@router.get("/list", response_model=List[HabitRead])
def read_habits(user_id: int = Depends(get_user_id), skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def create_habit(habit: HabitCreate, user_id: int = Depends(get_user_id), db: Session = Depends(get_db)):
  try:
...
def read_habit(habit_id: int, user_id: int = Depends(get_user_id), db: Session = Depends(get_db)):
  get_habit_or_err(db, habit_id, user_id)
  habit = crud_habits.get_habit(db, habit_id)
  return habit

@router.patch("/{habit_id}", response_model=HabitRead)
def update_habit(habit_id: int, habit_data: HabitUpdate, user_id: int = Depends(get_user_id), db: Session = Depends(get_db)):
  get_habit_or_err(db, habit_id, user_id)
