...
def test_get_water_goal_today():
  ...
  fake_goal = WaterGoal(
    user_id=1, ml_goal=2000, ml_drinked=500, last_updated=now
  )
...
def test_get_water_goal_resets_on_new_day():
  ...
  fake_goal = WaterGoal(
      user_id=1, ml_goal=2000, ml_drinked=1000, last_updated=yesterday
  )
  mock_db.query().filter().first.return_value = fake_goal
...
@patch("app.crud.crud_water_goal.get_water_goal_by_user")
def test_create_water_goal_success(mock_get_goal):
  mock_db = MagicMock()
  mock_get_goal.return_value = None
  data = WaterGoalCreate(weight=70)

  result = crud_water_goal.create_water_goal(mock_db, 1, data)

  mock_db.add.assert_called_once()
  mock_db.commit.assert_called_once()
  mock_db.refresh.assert_called_once()
  assert isinstance(result, WaterGoal)
  assert result.ml_goal == 2450  # 70 * 35

@patch("app.crud.crud_water_goal.get_water_goal_by_user")
def test_create_water_goal_default_value(mock_get_goal):
  mock_db = MagicMock()
  mock_get_goal.return_value = None
...
def test_create_water_goal_already_exists(mock_get_goal):
  mock_db = MagicMock()
  mock_get_goal.return_value = WaterGoal(user_id=1)
  data = WaterGoalCreate(weight=60)

  with pytest.raises(HTTPException) as exc:
    crud_water_goal.create_water_goal(mock_db, 1, data)
...
def test_update_water_goal_success(mock_create_intake, mock_get_goal):
  ...
  result = crud_water_goal.update_water_goal(mock_db, 1, data)
...
def test_update_water_goal_not_found(mock_get):
  mock_db = MagicMock()
  result = crud_water_goal.update_water_goal(mock_db, 1, WaterGoalUpdate())
  assert result is None
...
def test_delete_water_goal_success(mock_get):
  mock_db = MagicMock()
  fake_goal = WaterGoal(user_id=1)
  mock_get.return_value = fake_goal

  result = crud_water_goal.delete_water_goal(mock_db, 1)
