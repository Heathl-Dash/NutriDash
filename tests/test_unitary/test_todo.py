...
def test_get_todos_returns_sorted_results():
    ...
    now = datetime.now()
    fake_todos = [
        ToDo(id=1, title="Antigo a fazer", done=False, created=now - timedelta(days=3), keycloak_id=user1),
        ToDo(id=2, title="Recente feito", done=True, created=now, keycloak_id=user1),
        ToDo(id=3, title="Recente a fazer", done=False, created=now - timedelta(hours=1), keycloak_id=user1),
    ]
...
def test_update_todo():
  mock_db = MagicMock()
  fake_todo = ToDo(title="ir na nutricionista na sexta", description="", keycloak_id=user1)
